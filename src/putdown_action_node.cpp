...
using namespace std::chrono_literals;
  std::string confirm;
class PutdownAction : public plansys2::ActionExecutorClient
{
public:
  PutdownAction()
  : plansys2::ActionExecutorClient("putdown", 2s)
  {
    ...
    confirm_ = create_subscription<std_msgs::msg::String>(
      "/confirmation_topic",
      10,
      std::bind(&PutdownAction::confirm_callback, this, _1));
  }
 ...
 //creazione del publisher sul topic che verrà letto dal nodo del movimento    
    publisher_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/points/place", 10);
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<PutdownAction>();

  node->set_parameter(rclcpp::Parameter("action_name", "putdown"));
